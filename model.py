...
def build_model(conf: Config, space, input_shape) -> Model:
    ...
    x = Conv2D(
        ...
        padding=('same' if space['maxpooling_1'] is not None else 'valid')
    )(input_data)
    ...
    if space['filters_conv2d_7'] is not None:
        x = Conv2D(
            ...
            kernel_size=3,
            activation='relu',
            padding=('same' if space['maxpooling_7'] is not None else'valid')
        )(x)
        if space['maxpooling_7'] is not None:
            x = MaxPooling2D(pool_size=2, padding='same')(x)
        x = Dropout(space['dropout7'])(x)
    ...
    if space['filters_conv2d_8'] is not None:
        x = Conv2D(
            ...
            kernel_size=3,
            activation='relu',
            padding=('same' if space['maxpooling_8'] is not None else'valid')
        )(x)
        if space['maxpooling_8'] is not None:
            x = MaxPooling2D(pool_size=2, padding='same')(x)
        x = Dropout(space['dropout8'])(x)

    x = GlobalAveragePooling2D()(x)

    if space['dense_1'] is not None:
        x = Dense(space['dense_1'], activation='relu')(x)
        x = Dropout(space['dropout_dense'])(x)
    if space['dense_2'] is not None:
        x = Dense(space['dense_2'], activation='relu')(x)
        x = Dropout(space['dropout_dense'])(x)
    if space['dense_3'] is not None:
        x = Dense(space['dense_3'], activation='relu')(x)
        x = Dropout(space['dropout_dense'])(x)

    x = Dense(conf.num_classes, activation='softmax', name='dense_softmax')(x)
